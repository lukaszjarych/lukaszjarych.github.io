---
title: About
description: About Lukasz Jarych.
intro: A concise overview focused on strengths, direction, and how experience translates into Analytics Engineering.
---

I am a BI and analytics professional with 12+ years of experience across reporting, semantic modeling, SQL, Power BI, Azure, automation, and stakeholder-facing analytics delivery.

{% if site.data.settings.about_photo %}
<figure class="about-photo">
  <img src="{{ site.data.settings.about_photo | relative_url }}" alt="{{ site.data.settings.profile_photo_alt }}">
</figure>
{% endif %}

My recent portfolio direction is intentionally focused on Analytics Engineering roles. The goal is to show not only dashboard delivery, but also the design choices behind trusted analytics systems: transformation logic, model quality, governance, maintainability, and decision support.

Much of my professional work is confidential, so this portfolio uses carefully written summaries, selected sanitized visuals, and technical framing that stays accurate without disclosing sensitive implementation details.
